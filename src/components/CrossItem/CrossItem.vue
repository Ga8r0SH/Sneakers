<template>
  <div class="container">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 bg-white p-8">
      <div v-for="(sneaker, index) in sneakersStore.sneakers" :key="sneaker.id" class="sneaker-card relative">
        <div class="sneaker-image-wrapper relative ">
          <img :src="sneakersStore.photo[index % sneakersStore.photo.length]" :alt="sneaker.title"
            class="sneaker-image">
          <span @click="addFavoriteProduct(sneaker)"
            :class="{ 'favorite-added': sneakersStore.favorite.includes(sneaker) }"
            class="absolute top-4 right-4 p-2 rounded-full  transition duration-300 transform hover:scale-110">
            <svg class="heart align-middle" width="18" height="18" viewBox="0 0 16 16" fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path
                :class="{ 'hidden': sneakersStore.favorite.includes(sneaker), 'block': !sneakersStore.favorite.includes(sneaker) }"
                id="svg1"
        ...
        <div class="sneaker-info mt-4">
          ...
          <div class="flex justify-between items-center mt-2">
            ...
            <div class="text-lg font-bold">{{ sneaker.price }} Лей</div>
          </div>
          <button @click="addProduct(sneaker, sneakersStore.photo[index % sneakersStore.photo.length])"
            class="add-to-cart-button mt-4 bg-green-500 text-white py-2 px-4 rounded-md shadow-md transition duration-300 transform hover:scale-105">
            Добавить в корзину
          </button>
        </div>
      </div>
