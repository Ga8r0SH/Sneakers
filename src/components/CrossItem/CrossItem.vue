    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 bg-white p-8">
      <div v-for="sneaker in sneakersStore.sneakers" :key="sneaker.id" class="sneaker-card relative">
        <div class="sneaker-image-wrapper relative">
          ...
          <span @click="addFavoriteProduct(sneaker)" :class="{ 'favorite-added': sneakersStore.favorite.includes(sneaker) }" class="absolute top-4 right-4 p-2 rounded-full bg-white transition duration-300 transform hover:scale-110">
            <svg  class="heart align-middle" width="18" height="18" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
